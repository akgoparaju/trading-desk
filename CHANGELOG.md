...
- **QC5 — one beta, street-standard window. ⚠ Awaiting ratification.** Beta was computed over the *entire* ingested history (~6,730 daily returns, ~26 years) while the skill documentation claimed "12m vs SPY". Now **5-year monthly** (60 month-end log returns vs SPY): AAPL 1.1299 → **1.0781** (street 1.05–1.10, vendor 1.086); MU 1.6616 → **2.0437** (street 2.12–2.21, vendor 2.213 — the old value was the lowest of seven on a stock that 7×'d in a year). Disclosed via `beta_basis`/`beta_n_obs`/`beta_vendor`; `beta_n_days` deliberately keeps its calendar-days-spanned meaning because `score_risk` gates on its magnitude. **Scoring impact: MU risk −2 points** (crosses the 1.2–1.8 band into >1.8); AAPL unchanged. Coverage's WACC beta is hand-authored in `coverage/valuation.md` and reconciled to this value nowhere in code — that dependency is now stated in `beta_basis`, deliberately surfaced rather than changed.
