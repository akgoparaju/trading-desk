...
- **Redirect the workspace (`--output-dir`).** A caller whose process CWD is pinned to a foreign repo root (a sandboxed host where a `cd` doesn't persist between calls) can point an entire run's I/O — bundle, `trading_desk_config.json`, `coverage/`, adapters, scales, and the rendered report + PDFs — at an explicit absolute directory via `--output-dir <ABS_DIR>` on the entry skills (`full-trade-analysis` / `refresh-analysis` / `market-snapshot` / `company-context`). One root governs reads and writes; omitting the flag leaves output CWD-relative, unchanged.
