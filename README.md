...
**v1.0.0 — a calibrated rubric book, analysis depth, and a machine-consumable decision contract.** The full pipeline is wired end to end: the `market-snapshot` data engine (L1), the four evidence modules (technical, sentiment, risk, fundamental), the `composite-score` decision layer (L3), the `trade-plan` + `options-strategy` execution layer, the `report-renderer` 3-page output + docket with its blocking QC gate (L4), and the `full-trade-analysis` orchestrator (L5) that runs them all through phase gates.

The 0.14.0 depth roadmap deepened every evidence module to institutional-practice rubrics and added a scored **confidence layer** (see [Analysis depth & confidence](#analysis-depth--confidence)): event-aware risk, sentiment positioning dynamics, regime-conditional technicals, event-vol-aware options, and base-rate-anchored composite scenarios. **Every score-moving rubric shipped PROVISIONAL** — a versioned default with a *pre-registered falsifier*, ratified only against a calibration set. That set has now run (10 anchored names): it **structurally ratified the surviving rubrics** (the confidence depth promotes where earned), **recalibrated the one threshold whose falsifier tripped**, and **re-based two rubric branches** whose scoring windows didn't fit real names — while OUTCOME predictiveness (do the scores predict forward returns) stays honestly forward-tracking in a decision ledger. Reports disclose what's still provisional in the footer and confidence badge; nothing reads as settled that isn't.
...
- **Redirect the workspace (`--output-dir` / `--prev-dir`).** A caller whose process CWD is pinned to a foreign repo root (a sandboxed host where a `cd` doesn't persist between calls) can point an entire run's I/O — bundle, `trading_desk_config.json`, `coverage/`, adapters, scales, and the rendered report + PDFs — at an explicit absolute directory via `--output-dir <ABS_DIR>` on the entry skills (`full-trade-analysis` / `refresh-analysis` / `market-snapshot` / `company-context`). Under `--output-dir` the workspace is **flat** (that dir IS the ticker workspace — no `trading_desk_<T>/` subfolder), so a per-run dated dir stays clean. A refresh into a fresh dated dir reads the previous run from a separate read-only workspace via `--prev-dir <ABS_DIR>`. One root governs reads and writes; omitting `--output-dir` leaves output CWD-relative and nested, unchanged.
...
**Provisional by design — structurally calibrated, outcome still forward-tracking.** The rubric weights and bands shipped as *provisional defaults*, each with a pre-registered falsifier, disclosed as unratified in the module note, the report footer, and the confidence depth axis. A 10-name calibration set has since run against those falsifiers: it **structurally ratified the surviving rubrics** (risk's confidence depth promotes to HIGH where the event/tail separation earned it), **recalibrated the one threshold whose falsifier tripped** (the composite auto-tension spread), and **re-based two rubric branches** whose scoring windows didn't fit real names (the tail calm-tier now scores a trailing-3y magnitude window with full-history kurtosis demoted to a diagnostic; the crowded-short signal now uses a corrected days-to-cover basis plus a joint SI×DTC condition). The OUTCOME half — do the scores and bands PREDICT realized forward returns — needs forward time and runs as a decision ledger; pre-deploy it reads honestly as forward-tracking, not settled. The system still tells you exactly what it hasn't yet earned the right to assert.
