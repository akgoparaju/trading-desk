...
description: Build a verified, QC-gated market data snapshot for a ticker from Alpha Vantage (single source of truth for all downstream trading-desk skills). Use when the user says "snapshot [ticker]", "market snapshot", "build data snapshot", or when any trading-desk skill needs market data. Schema v0.7.0.
---
...
11. `INSIDER_TRANSACTIONS` symbol=`<T>`, **from_date = as-of − 36 months** → (`insider_transactions`). Widened from 90 days (Wave 3A): the builder still scores a trailing-90d net flow, but the Cohen/Malloy/Pomorski routine-vs-opportunistic classifier needs ≥24 months of per-insider history to separate routine (same-calendar-month, ≥2 prior years) from opportunistic trades. Fetching 36 months gives the classifier its history so future runs carry it; **graceful when the vendor returns less** — `build_insider_classification` sets `classifier_active: false` when `history_months < 24` and the scorer falls back to the unchanged net-90d logic. (Disclosed: the extra rows are used only by the classifier; the net-90d dollar math is still window-clipped to 90 days.)
