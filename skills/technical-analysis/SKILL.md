...
description: Score a ticker's technical setup (trend, momentum, structure, volume/extension) against a versioned rubric, off an existing market-snapshot bundle. Use when the user says "technical analysis [ticker]", "chart check", "support and resistance", "is it overbought", or when a report needs the technical evidence module. Consumes an existing snapshot bundle in the CWD (runs market-snapshot first if none exists). Rubric v1.3.0 (PROVISIONAL: regime guard + volume-quality bands + sector-relative RS + short-horizon trend + extension-credit gating).
---
...
> **Rubric v1.3.0 is PROVISIONAL (Wave 4A · R5 / B28 + Track O4; QC20(a)+(c)).** Top-level weights of the four cores are UNCHANGED (Trend 30 / Momentum 25 / Structure 25 / Volume 20). v1.1.0 adds a **regime GUARD** on momentum (ADX + Weinstein stage: a choppy `adx14 < 20` halves the MACD sub-component; a declining `stage == 4` caps the RSI healthy-band bonus at 12) — the guard MODULATES existing sub-scores, it is not a new scored factor and its band shapes are unchanged. It also **re-splits the volume factor** (extension 10 / vol-regime 5 / A/D-line slope 3 / up-day volume 2, summing to the same 20) and adds **anchored-VWAP levels** (`vwap_52wk_high`, `vwap_earnings`) to the S/R ladder as institutional cost-basis support candidates. v1.2.0 (Track O4) adds a **new scored sector-relative RS factor** (see the Built disclosure below). The guard thresholds, the A/D + upvol volume-quality bands, and the sector-RS bands are **documented Philosophy-A defaults; the B9 structural set (2026-07-23) confirmed they FIRE at sane rates and separate cleanly** — the choppy guard fired 5/10, A/D + up-vol spread across buckets, and 9/10 names' sector-RS spread across the bands; the pre-registered falsifier survived, and band-vs-forward-winner predictiveness stays outcome-forward-tracking. The module JSON carries a `module_note` with the status. The confidence DEPTH badge reads HIGH at v1.1.0 (the regime-depth pass has landed; technical's source is AV-premium, not web-capped) — this answers "has the depth pass landed?", distinct from "are the thresholds calibrated?" (which the module_note discloses is still provisional). Null `adx14`/`stage` → no modulation (graceful; a pre-4A snapshot scores as before). **v1.3.0 (QC20(a)+(c)) adds two more factors, both shipped behind pre-registered falsifiers that had already passed before implementation began:** a `trend_short` sub-factor (max 10, split 3/3/2/2 across `ma10` level, `ma21` level, and their 5-day slopes) that can see a break the slower top-level `trend` factor structurally cannot, days after it happens; and `technicals.extension_gate`, a cap wired into `score_volume` — a session moving ≤ −2.0σ within 3 sessions of a `reportedDate` earnings print caps extension credit at its pre-break value for the following 10 sessions, so a name no longer earns extension points FOR crashing. Both are named PROVISIONAL and carry forward falsifiers (grade-change rate on the B9 set; an outcome test) — see `CHANGELOG.md` 1.4.0 for the measured AAPL/MU impact.
>
...
2. **A single paragraph, ≤150 words (aim ~120).** Wrap the paragraph in `<!-- BRIEF:START -->` … `<!-- BRIEF:END -->` delimiters (one delimiter per line, the paragraph text between them). Cite ONLY numbers present in `module_technical.json` (the `subscores[].arithmetic` strings and `inputs`) or the snapshot. Zero computed-in-prose numbers. Write complete sentences in plain English — the brief is read by a human in the final report. Spell out metric names ("RSI 36.1", "50-day MA"), never snake_case identifiers or `->` arrows; write large magnitudes as $16.7B, never scientific notation. Round displayed floats to 2 dp (full precision stays in the JSON). Walk the four dimensions (trend, momentum, structure, volume/extension), naming the points each earned and why, using the `arithmetic` strings as your source of truth.
...
- **Rubric version travels with the numbers.** The rubric version (`1.3.0`) is printed in the module JSON and MUST appear in the brief footer, so any reader can tell which scoring rule produced the score. When `module_note` is present (v1.3.0 PROVISIONAL), surface it near the footer so the reader knows the regime guard + volume-quality bands + sector-relative RS factor + short-horizon trend/extension-credit gating are unratified.
