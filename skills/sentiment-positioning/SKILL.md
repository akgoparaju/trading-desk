...
The bands are **cited defaults; the B9 structural set (2026-07-23) confirmed the TESTED bands (balanced/moderate skew, bull/neutral news-heat, the `dtc<2` notch) fire correctly** (the falsifier survived); predictiveness stays outcome-forward-tracking. **Crowded-short notch RE-BASED (2026-07-23):** the `dtc>10`-only notch rarely fired — high-SI names are high-volume, so days-to-cover stays moderate (a ~30%-short name read dtc≈6-7). So DTC's data basis was corrected (a **direct 3m share-volume ADV** denominator, not `adv_dollar_3m/last` which distorts DTC for names whose price moved; + the **SharesFloat** base for the short-share count, disclosed via `sentiment.dtc_basis`) and the notch now fires on **`si_trend rising AND (dtc>10 OR (si_pct>15 AND dtc>5))`** — catching a heavily-shorted, still-covered book. **Still unexercised** on real names: the extreme-skew / bearish-news bands did not fire even on a crashing, heavily-shorted name.
...
**Confidence stays honestly MEDIUM** — deeper rubric, but sentiment SOURCE is structurally capped at MEDIUM anyway (short interest is web-transcribed by design), so the badge does not rise. Surface the provisional status; do not overstate it.

## PROVISIONAL — sentiment-v1.2.0 (QC17, ratified 2026-08-10)

**QC17 re-keys and re-cuts the revisions-momentum leg (max 20).** `score_revisions` now reads `fundamentals.revisions_ntm.pct` — the same NTM-blended consensus `valuation.pe_fwd` is already scored against — when present (basis `ntm_blend`), falling back to `fundamentals.revisions_90d.pct` (basis `near_fy_fallback`) when `revisions_ntm` is null/absent, e.g. a thin-estimate name with only one future fiscal-year row such as COHR. **The basis actually used is always named in the subscore's `arithmetic` string** — never silent, and worth citing in the brief when it is the fallback. One band grid serves both bases (a dual grid would create a discontinuity the day a name's `revisions_ntm` blend picks up a second FY row): the edges widened from `>0.03 / >0.005 / ≥−0.005 / ≥−0.03` to `>0.15 / >0.01 / ≥−0.01 / ≥−0.15` after a 21-name sweep found the old edges saturating 61.9% of names into the top band. The point ladder, ±2 count adjustment, cap/floor, and null handling are unchanged. Both outer edges are documented guesses — `+0.15` is fitted to the measured cross-section, `−0.15` is a pure symmetry guess with zero supporting observations — with quarterly falsifiers pre-registered (F17-A2 forward saturation, F17-D modal concentration, F17-E regime robustness, F17-F the empty bottom band); see `docs/reviews/2026-08-10-qc17-band-recut-proposal.md`.

The module JSON stamps `rubric_version: "1.2.0"` and `module_note: "sentiment-v1.2.0 PROVISIONAL — positioning/news bands unratified pending B9; revisions-momentum re-cut to the NTM basis (QC17), edges unratified pending quarterly falsifiers F17-A2/D/E/F; falsifier pre-registered"`.
...
- **Rubric version travels with the numbers.** The rubric version (`1.2.0`, PROVISIONAL) is printed in the module JSON and MUST appear in the brief footer, so any reader can tell which scoring rule produced the score. When `module_note` is present (it is on v1.2.0), surface its provisional wording in the brief so a reader knows the positioning/news bands AND the revisions-momentum band re-cut (QC17) are unratified.
- **Revisions-momentum basis (QC17).** The `revisions_momentum` subscore's `arithmetic` always names which leg it scored — `basis=ntm_blend` (the default) or `basis=near_fy_fallback` (when `fundamentals.revisions_ntm` is null/absent). When the brief walks the revisions-momentum dimension, cite the basis actually used rather than assuming `ntm_blend`.
