...
- **Complacency guard.** In the positioning dimension, `short_interest_pct < 1.5%` combined with `rsi14 > 70` is scored as **complacency (2 pts), not a bullish signal** — a stock nobody is shorting while it is overbought has priced out the doubters, which is a crowded-long risk, not a squeeze setup. The guard fires *before* the normal short-interest bands. `rsi14` only *conditions* this guard here; it is scored in technical-analysis, never double-counted here (it is a GUARD field, not an INPUT field).
