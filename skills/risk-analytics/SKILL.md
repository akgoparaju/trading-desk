...
**Higher score = BETTER conditions.** A high risk-analytics score means the *setup* is favorable (low volatility, shallow historical drawdowns, a real discount, tight downside vs. upside, deep liquidity, a strong balance sheet) — it does NOT mean the stock is dangerous. State this convention as a short note in the score HEADLINE line (not inside the ≤150-word paragraph, where it eats the word budget).
...
**Why provisional (user philosophy "A"):** the event/tail weights + bands are a versioned, cited, falsifiable DEFAULT — shipped now, ratified only after the B9 calibration set (5–10 anchored names) runs. Until then `rubric_version` is `1.1.0` and the module note reads `risk-v1.1.0 PROVISIONAL — event/tail weights unratified pending B9 calibration; falsifier pre-registered`. **Confidence DEPTH for risk was PROMOTED MEDIUM→HIGH on the B9 structural ratification (2026-07-23)** — the 10-name calibration set confirmed `event_risk` points are monotone in days-to-event (near+loud events discounted, far events at max) and `tail_risk` separates by kurtosis; the pre-registered falsifier survived. The OUTCOME half — does the event weight PREDICT realized gaps — remains forward-tracking in the decision ledger. **Tail calm-tier RE-BASED (2026-07-23):** the calm tier used to be unreachable because kurtosis was measured over the full (~26-yr) gap history — even the calmest staple read violent. `tail_risk` now scores the **trailing-3y gap regime by MAGNITUDE** (`p95_abs_3y` + worst-5% `tail_mean_95_3y`, both must clear the cut: calm <1.5%/<3% · moderate <4.5%/<7% · else violent), and full-history `excess_kurtosis` is a **diagnostic**, not a gate — so a lone decades-old crash gap no longer brands a currently-calm name violent (a large-cap staple now reads calm).
...
- **C-ID citation requirement (top_risk).** When `module_context.json` exists in the bundle and you pass `--top-risk` with `--stress-pct`, the `--top-risk` string MUST cite ≥1 context finding ID (`C<n>`, e.g. `C1`) from `module_context.findings[]` that grounds the named risk. A `top_risk` assertion with no `C<n>` anchor is ungrounded. The `report_qc` gate (`judgment_flag_citations` check) enforces both grounding (≥1 C-ID present) and referential integrity (every cited C-ID exists in the registry) — a failure blocks the report. On the compressed floor (no `module_context.json`), or when `--stress-pct` is omitted, this requirement does not apply and the check auto-passes for this flag.
