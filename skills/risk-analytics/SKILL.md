...
**Why provisional (user philosophy "A"):** the event/tail weights + bands are a versioned, cited, falsifiable DEFAULT — shipped now, ratified only after the B9 calibration set (5–10 anchored names) runs. Until then `rubric_version` is `1.1.0` and the module note reads `risk-v1.1.0 PROVISIONAL — event/tail weights unratified pending B9 calibration; falsifier pre-registered`. **Confidence DEPTH for risk was PROMOTED MEDIUM→HIGH on the B9 structural ratification (2026-07-23)** — the 10-name calibration set confirmed `event_risk` points are monotone in days-to-event (near+loud events discounted, far events at max) and `tail_risk` separates by kurtosis; the pre-registered falsifier survived. The OUTCOME half — does the event weight PREDICT realized gaps — remains forward-tracking in the decision ledger. **Known limitation the set surfaced:** the `tail_risk` *calm* tier (8 pts, `excess_kurtosis < 8`) is effectively unreachable because kurtosis is measured over the full (~26-yr) overnight-gap history — even the calmest staple reads violent; flagged for a follow-up tail-lookback calibration.
