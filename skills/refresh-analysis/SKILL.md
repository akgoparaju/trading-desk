...
- **`sector_daily_adjusted`**, when it appears in the plan at all (a normal window-based refetch, OR reason `gap-fill: sector ETF resolved, absent last run` — this ticker's sector maps to a real SPDR ETF but the series was never fetched before): fetch it per market-snapshot **Step 4b**'s convention — resolve the ETF from the fresh (or carried) `overview.Sector` with `resolve_sector_etf` and `TIME_SERIES_DAILY_ADJUSTED` that symbol, outputsize=full → (`sector_daily_adjusted`). If the group is **absent from the plan entirely**, this ticker's sector has no SPDR-ETF mapping — skip it, exactly as market-snapshot does (a disclosed, legitimate absence, not a gap).
