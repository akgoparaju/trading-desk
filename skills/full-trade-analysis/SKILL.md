...
**FSI-workflow override (explicit).** The FSI initiating-coverage SKILL is single-task, ask-first, and produces .xlsx/.docx artifacts. This orchestrator deliberately OVERRIDES that gating: run its Tasks 1-3 content back-to-back without per-task user gates, at proportionate depth (the model feeds valuation anchors, not a 10-tab workbook), and direct markdown-first artifacts into `coverage/` (research.md, model.md + computed JSONs, valuation.md). Disclose any scoping-down in the coverage files themselves. This override is intentional — do not refuse to chain on the FSI SKILL's own single-task rule when invoked from this phase.
