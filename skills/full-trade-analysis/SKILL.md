...
- **Ticker workspace — FLAT under `--output-dir` (v1.2.0):** when `--output-dir` is given, `<WORKROOT>` **is the ticker workspace directly** — `TICKER_WS = <WORKROOT>` (drop the `trading_desk_<TICKER>/` segment; the caller already passes a per-ticker `--output-dir`, so that collision boundary is redundant). When absent, keep the nested `TICKER_WS = ./trading_desk_<TICKER>` (the human multi-ticker-in-one-CWD layout — byte-for-byte unchanged).
- Everywhere below that reads/writes `./trading_desk_<TICKER>/…` (the bundle, `coverage/`, the rendered report/PDFs) → use `<TICKER_WS>/…`. The bundle is `BUNDLE = <TICKER_WS>/detail_reports_<YYYY-MM-DD>`. The **config + scales/adapters stay at the workspace root** regardless of layout: `<WORKROOT>/trading_desk_config.json` and `<WORKROOT>/trading_desk_config/…`.
- When you invoke a **sub-skill** (`market-snapshot`, `company-context`, `report-renderer`), pass **`--output-dir <WORKROOT>`** along.
- When you dispatch **evidence subagents** or emit a `python3 scripts/…` call, give every path argument (`--bundle`, `--anchors`, `--adjusted`, `--scale`, `--weights-config`, `--coverage`) as an **absolute path built from `<TICKER_WS>`/`<WORKROOT>`** — the downstream scorers then never fall back to the CWD.
...
Look for `<TICKER_WS>/coverage/` (the FSI initiation artifacts — research / model / valuation; a programmatic caller may have symlinked it here) and branch:
...
**The coverage DCF contract (QC-D1, owner-ratified 2026-08-10 — forward-looking; nothing retroactive, existing bundles stay valid as shipped).** Every DCF newly authored or refreshed in `coverage/valuation.md` from this date follows three conventions:

- **Fade horizon — ADOPTED.** Extend the explicit fade until the terminal value's share of enterprise value lands inside the desk's own 50–70% discipline band (the same band the FSI `dcf-model` skill itself states: *"terminal value should represent 50-70% of Enterprise Value"*). A bundle that ships a terminal share outside that band — AAPL's shipped 5-year horizon self-flagged 71.9% — has not cleared its own gate; lengthen the fade until it does, not the other way around.
- **One governed cost-of-capital build — ADOPTED.** `WACC = rf + beta × ERP`. `rf` is the bundle's own `treasury_yield` (never invented). `beta` is the desk's ratified beta — the SAME 5-year-monthly-simple beta the snapshot scores (`benchmark.beta`), never a separately hand-authored figure. `ERP = 5.50%`, **held constant** — the FSI skill's own 5.0–6.0% band midpoint, held because no governed ERP source exists anywhere in the desk's data plane; say so plainly in `valuation.md` rather than presenting 5.50% as a retrieved figure. **A judgmental deviation from any of the three inputs is allowed but MUST be named as a deviation and priced** in `coverage/valuation.md` (e.g. "at the vendor's unadjusted beta of X, WACC moves to Y% and `dcf_base` to $Z") — an un-named deviation is exactly the per-bundle drift this convention exists to close.
- **SBC as a cash operating expense — HELD.** Unchanged from the AAPL coverage's own argument; it is now a decided house convention, not an open question. State it explicitly in every bundle — silence is not a convention.
...
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/coverage_qc.py \
  --coverage <TICKER_WS>/coverage --mode full
```
...
1. **The bundle path** — `<TICKER_WS>/detail_reports_<YYYY-MM-DD>` **as an absolute path** (mandatory when `--output-dir` was given so the subagent writes into the workspace, not its own CWD; legacy `<WORKROOT>/td_bundle_<TICKER>_<date>` bundles also resolve via the discovery glob).
...
- **Fundamental `--anchors <TICKER_WS>/coverage/valuation_anchors.json`** — pass it **when the file exists** (the `coverage_distilled` path where Phase 0.5 transcribed it). This switches the fundamental valuation dimension to anchored mode (DCF/comps/own-history/fcf/justified-band, PEG display-only). Absent (the `web_compressed` floor), omit it — snapshot-mode valuation. Malformed → exit 2 (Phase-0.5 validation backstop). (The **risk** module reads the SAME anchors file — wired in Phase 2's risk-analytics dispatch — for its `dcf_bear` downside floor; one coverage-derived anchor set feeds both consumers.)
- **Fundamental `--adjusted <TICKER_WS>/coverage/adjusted_financials.json`** (O14) — ALSO pass it **when that file exists** (Phase 0.5 transcribed a material one-time item). It normalizes the scored ROE + forward-P/E onto the FSI's clean figures and discloses GAAP-vs-core. Absent → GAAP behavior (no change). It is OPTIONAL and independent of `--anchors`.
...
Invoke the **report-renderer** skill for the bundle (**pass `--output-dir <WORKROOT>`**): `render_report.py` writes the full 3-page skeleton (every table/number script-owned); you fill only the `<!-- SLOT:... -->` prose slots citing numbers already printed on the page; then `report_qc.py` runs the **blocking §12 gate** (`--report <path>`), which must exit 0.

**GATE — report QC (`report_qc.py` exit 0, BLOCKING).** Fix the **prose**, never the numbers: `no_empty_slots` → fill the slot; `number_provenance` orphan → remove/rephrase to a printed figure (never invent a number); `word_cap` → cut your own prose (the count excludes tables and the Data Integrity section, so the overage is always yours); `footer_completeness` → restore the deleted `api_tier_notes` entries — the Data Integrity section is mandated disclosure and is never trimmed; a table-driven check failing (composite_arithmetic / ev_consistency / sizing / strikes / pop_method) is an upstream module bug — fix the module and re-render. A genuinely justified failure may be `--waive "check:reason"` (disclosed, never to hide a fabricated number). Re-run until exit 0.
...
**Deliver:** the **report path** — `render_report.py` derives it from `--bundle` and writes it to the **ticker parent** `<TICKER_WS>/<TICKER>_Trade_Report_<date>.md` (a sibling of the `detail_reports_<date>/` data folder; legacy bundles keep it inside), printed to stdout — plus the **composite line** (`grade — action, score/100, profile`) + the **expression line** (recommended structure/size for the profile) + the **coverage line** (`coverage_distilled` vs `web_compressed`, and "initiation run this session" if Phase 0.5 (b) fired) + the **QC attestation** (gate verdict).
