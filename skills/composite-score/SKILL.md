...
- **Base-rate anchoring + auto-tension are PROVISIONAL (composite-v1.1.0).** The ±5% material-move bins, the 25pp deviation threshold, the N≥4 minimum history, and the 25-pt auto-tension spread are review's first numbers (Philosophy A), stamped `PROVISIONAL` in the module `note`. Both surfaces are **disclosed, never hard gates**: the base-rate flag makes an overconfident probability set visible without blocking it; auto-tension surfaces a wide evidence spread. Say so if a reader treats either as calibrated.
