...
Then tell them: the new plugins load in the NEXT session — this run continues with the compressed pass, and the next analysis will use deep FSI mode automatically.
    ...
    **The compressed-without-context floor is the last resort.** ONLY when no `module_context.json` exists (the FSI-absent floor where the context module ran `web_compressed` but produced no registry to cite, or context was skipped entirely), run the scorer with **`--moat` omitted**:
    ...
    Then **write `<bundle>/brief_fundamental.md`** — fundamental has no standalone skill, so the composite step owns its evidence brief. Read `module_fundamental.json` directly and write the brief in the **same format as the other evidence briefs** (technical / sentiment / risk), in order:
      ...
      2. **A single paragraph, ≤150 words (aim ~120).** Wrap the paragraph in `<!-- BRIEF:START -->` … `<!-- BRIEF:END -->` delimiters (one delimiter per line, the paragraph text between them). Cite ONLY numbers present in `module_fundamental.json` (the `subscores[].arithmetic` strings and `inputs`) or the snapshot — zero computed-in-prose numbers. Write complete sentences in plain English — the brief is read by a human in the final report. Spell out metric names ("ROE 18.4%", "forward P/E 22.1x"), never snake_case identifiers or `->` arrows; write large magnitudes as $16.7B, never scientific notation. Round displayed floats to 2 dp (full precision stays in the JSON). Walk the sub-dimensions (quality, valuation), naming the points each earned and why, using the `arithmetic` strings as your source of truth. **State the mode disclosure**: if `fundamental_mode` is `compressed_snapshot_pass`, say the pass was snapshot-only (deep model reuse not applied).
      3. **One-line signal** — Wrap the signal in `<!-- SIGNAL:START -->` … `<!-- SIGNAL:END -->` delimiters (one delimiter per line, the signal text between them). Your single-sentence read of the fundamentals (prose, never a number).
...
**ENFORCEMENT — `report_qc` `judgment_flag_citations` check (B29).** The `report_qc` gate enforces both grounding and referential integrity for the three composite conviction flags (`--variant-justification`, `--catalyst-clarity-justification`, `--invalidation-justification`) at report time: every non-default flag's justification must contain ≥1 `C<n>` token (grounding), and every cited `C<n>` must exist in `module_context.findings[]` (referential integrity). A failure — ungrounded justification or an orphan C-ID — blocks the report (exit 1). When `module_context.json` is absent (the compressed floor), the check auto-passes. Waivable with `--waive "judgment_flag_citations:reason"` (disclosed, never to hide a fabricated citation). The `--invalidation-justification` is also checked here even though the invalidation gate does not regex it at score time — this is the ordering-safe enforcement point (Phase 5, context guaranteed present).
...
2. **The one-line tension sentence** — Wrap the tension paragraph in `<!-- BRIEF:START -->` … `<!-- BRIEF:END -->` delimiters (one delimiter per line, the sentence between them). Write it as a complete sentence in plain English — spell out metric names, never snake_case identifiers or `->` arrows, and large magnitudes as $16.7B, never scientific notation; round any displayed float to 2 dp. The strongest bull point vs the strongest bear point in a single sentence (e.g. "The setup rewards the HBM ramp (technical 70, clear catalyst) but the below-median valuation floor and a middling risk read cap the conviction — a hold, not an add."). **`tension` now AUTO-POPULATES** (composite-v1.1.0, Goal C): when the spread across the four evidence-dimension scores (`thesis_conviction` excluded) exceeds **25 points**, the script writes a scripted string naming the high/low dims + spread (e.g. `"sentiment 58.8 vs fundamental 30.8 — 28-pt evidence spread"`) into `tension`; below threshold it stays `null`. When the JSON `tension` is non-null, quote it and you MAY layer a richer sentence on top; when it is `null`, write the tension sentence yourself. Either way the numbers you cite come from the `dimensions` rows.
...
7. **Mode disclosures** — if the fundamental module was the compressed pass, note it. Note any renormalization. **Base-rate check (Goal A, composite-v1.1.0):** read `flags.base_rate_check`. When `flagged: true`, disclose it in one line — the LLM scenario probabilities deviate from the ticker's own empirical earnings-move base rates by more than the 25pp threshold (quote `base_rates` and `deviations`); this is a **SOFT flag**, not a hard gate — a differentiated view is allowed, but the reader must see the gap and your reasoning for holding it. When `skipped: true` (history < 4 quarters), say the base-rate check was skipped for thin history (quote `skip_reason`). When `flagged: false` and not skipped, one line that the probabilities are in line with the empirical base rates.
...
- **Base-rate anchoring is disclosed + OUTCOME-forward-tracking; auto-tension was RECALIBRATED on the B9 set (composite-v1.1.0).** The ±5% material-move bins, the 25pp deviation threshold, and the N≥4 minimum history are review's first numbers — **disclosed, never a hard gate** (the base-rate flag makes an overconfident probability set visible without blocking it), and their predictiveness is forward-tracking. The **auto-tension spread was RECALIBRATED 25→35 pt on 2026-07-23**: the 10-name B9 set fired the 25 pt threshold on 6/10 (>half — refuting it per the falsifier); 35 pt fires on 3/10 (the genuinely high-tension names, on the natural break in the spread distribution). Auto-tension surfaces a wide evidence spread — a disclosure, not a hard gate.
- **Falsifier (composite-v1.1.0).** *If across the B9 set the base-rate deviation flag fires on names where the LLM view is later validated (i.e. flags good judgment as anomalous) more often than it catches genuine overconfidence, the 25pp threshold / ±5% bins are refuted and re-set; if auto-tension fires on >half the book or never fires, the 25-pt spread threshold is refuted.*
- **Rubric version travels with the numbers.** The rubric version (`1.1.0`) is printed in the module JSON and MUST appear in the brief footer, so any reader can tell which scoring rule produced the call.
