...
- **`<REPORT>` exists → SKIP the render.** Announce it loudly, unmissable to a human who never expected a resume: `resumed existing report at <REPORT> — your prior prose was KEPT, not regenerated. Wanted a fresh draft instead? Say so and I'll re-run with --fresh-skeleton, which discards the authored prose and re-renders from scratch.` Go straight to Step 3 (fill any open slots) and Step 4 (QC). **Do not write into the bundle at this step** — in particular `module_decision.json` stays exactly as the original render left it. (Step 5 still writes `pdf_slots.json` and `charts/` normally; the prohibition is on re-rendering, not on the docket.) This is the recovery entry point — a bundle can be complete and QC-passing while the report is a trim away from shipping, and re-rendering would discard every authored word and rewrite `module_decision.json` for nothing.
